    ...
    <>
      <div className='from-slate-900 to-slate-800 bg-gradient-to-tl rounded-xl   p-5  w-full text-white'>
        <div>
          ...
          <p className='text-sm text-slate-400'>Cryptocurrency is a digital or virtual currency that uses cryptography for security and operates on a <br />decentralized network. It allows for secure, fast, and low-cost transactions without the need for a central authority.</p>
        </div>
        {loading ? (
          <p>Loading...</p>
        ) : (
              ...
              {prices.map((coin) => (
                <tr key={coin.name} className='bg-slate-950 text-sm' >
                  <td className='flex gap-3 p-2'>
                    ...
                    />
                    <p>  {coin.name}</p>
                    <div className='bg-slate-900 text-xs p-1 rounded-sm uppercase text-blue-900'>
                      <p>{coin.symbol}</p>
                    </div>
                  </td>
                  <td>
